...
class DrawingPanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		drawArrowPosition(g);
		/*
		 * for(int i=0; i<obstacles.size(); i++){ drawObstacleLine(g,
		 * obstacles.get(i)); }
		 * 
		 * for(int i=0; i<obstacles.size(); i++){ drawObstaclePoint(g,
		 * obstacles.get(i)); }
		 */
	}
	...
	private void drawXYPoint(Graphics g) {
		...
		map.append(new MapData((posX / MAP_SQUARE_LENGTH) * MAP_SQUARE_LENGTH,
				(posY / MAP_SQUARE_LENGTH) * MAP_SQUARE_LENGTH, false));
	 ...
	 * g.setColor(new Color(0,0,255)); g.fillOval(point[0]-(pointSize/2),
	 * -point[1]-(pointSize/2), pointSize, pointSize); }
	 * 
