...
public final class Navigator
{
	...
	public void addSensorData(int distance) {
		...
		arc.setArcByCenter(data.getPositionX(), data.getPositionY(), distance - SAFETY_DISTANCE, data.getHeading() - US_SPREAD, data.getHeading() + US_SPREAD, Arc2D.PIE);
		addAllTiles(arc, false);

		if (distance < 255) { // Obstacle sensed
			arc.setArcByCenter(data.getPositionX(), data.getPositionY(), distance, data.getHeading() - US_SPREAD, data.getHeading() + US_SPREAD, Arc2D.PIE);
			addAllTiles(arc, true);
		}
	}
